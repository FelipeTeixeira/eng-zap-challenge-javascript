...
describe('PropertyService', () => {
    ...
    it('findProperty() should return data', () => {
        const PROPERTY_KEY = service.getStateKey('viva-real');
        const transferState = new TransferState();
        transferState.set(PROPERTY_KEY, propertiesData);

        service.findProperty(propertyData.id, 'viva-real').subscribe((res) => {
            expect(res).toEqual(propertyData);
        });
    });

    it('findPropertiesSimilar() should return data', () => {
        const PROPERTY_KEY = service.getStateKey('viva-real');
        const transferState = new TransferState();
        transferState.set(PROPERTY_KEY, propertiesData);

        service.findPropertiesSimilar(propertyData.id, 'viva-real').subscribe((res) => {
            expect(res).toEqual(propertiesData);
        })
    });
